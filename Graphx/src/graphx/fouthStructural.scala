...
object fouthStructural {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // graph.triplets.map(triplet=>triplet.srcId+"的"+triplet.srcAttr._1+"is the"+triplet.attr+"is" +triplet.dstId+"的"+triplet.dstAttr)
    ...
    //validGraph1.edges.collect().foreach(print(_))
    val ccGraph=graph.connectedComponents()
    //ccGraph.vertices.collect().foreach(print(_))//节点变成
    //ccGraph.edges.collect().foreach { n => print(n) }
    val validGraph = graph.subgraph(vpred = (id, attr) => attr._2 != "Missing")
    // Restrict the answer to the valid subgraph
    val validCCGraph = ccGraph.mask(validGraph)
    validCCGraph.vertices.collect().foreach(print(_))
    validCCGraph.edges.collect().foreach { n => print(n) }
  }
}